...
Trusted programmatic tokenizers receive only the query plus pre-filtered candidate content and normalized tags. Empty-input preflight, actual-array checks, 100,000-token and 256-UTF-16-unit limits, immediate copying, and fixed `TOKENIZATION_FAILED` errors bound ordinary failures without retaining an upstream cause. Search closes only lexical ranking and emits `lexical:tokenizer-unavailable`; if semantic also fails, it returns empty channels with both diagnostics. Reconciliation may use lexical alone; semantic-only fallback additionally requires a non-zero query vector and at least one non-zero candidate vector. Without that signal it degrades the accepted job and preserves recallable L1. Caller abort remains an abort. Because the extension is synchronous, a trusted implementation that never returns cannot be preempted at this boundary.
