...
Every add first commits a recallable L1 raw record and an `accepted` job. Direct mode then creates one L2 fact or L4 identity. Extraction mode sends untrusted conversation JSON to the configured model and accepts only an exact schema-valid JSON object. Extraction and reconciliation prompts include JSON Schemas generated from the same Zod validators; optional fields must be omitted when unknown, and invalid types still degrade the write without coercion. Each sanitized source independently ranks active, recallable, currently valid same-layer records from the owner row; Session is not a reconciliation boundary. Empty shortlists become deterministic `ADD` operations, while the model sees only non-empty sources and must cover each of those exactly once. In a trained remote embedding space, the first raw commit carries a same-space, same-dimension zero placeholder so no fallible network call can precede durable evidence. A successful commit replaces placeholders with validated, L2-normalized vectors; embedding failure degrades the job without derived records and leaves L1 lexically recallable.

Model routing is resolved only after the raw commit. An explicit non-empty `provider`/`model` pair stays fixed; omitting both reads the live Harness `agentDefaultModel` selection once per write. Extraction and reconciliation share that detached route even if Settings change mid-write. Missing default-model service degrades extraction while retaining raw evidence; direct writes and reads remain available. The bundle depends on `agentDefaultModel` without copying its startup values into plugin config.
...
**Make memory tools own the store.** Automatic capture, host consumers, migration, and UI consumers need a stable non-model capability. The tools remain a narrow optional consumer.
...
## Management and diagnostic boundaries

`/management` is an optional Host consumer requiring Connection/WebServer. The native `/client` factory registers a `settings.section` page. Every HTTP request uses Connection authentication and request-trust checks. Presets derive from configured tenant/user ownership and existing records; RPC rejects unknown fields and browser-supplied owner identities. Record DTOs use an explicit field allowlist: content and direct evidence are visible, while vectors, arbitrary metadata, raw warnings and exception bodies are omitted. This grants local deployment administration, not multi-tenant authenticated identity mapping.

Confirmation/correction uses the owner queue and whole-scope expectedRevision, validating the target and content before adding raw evidence. Idempotent retries precede revision checks. Embedding runs after accepted raw evidence is durable; success atomically commits a new explicit chain head, reciprocal old-version links and a completed job. Failure retains the old head and recallable raw. Confirmation preserves content; correction changes it; both create new revisions without overwriting history. Soft deletion reuses evidence-cascade semantics with an optional expectedRevision check for management calls.

Job timestamps, logical model-call counts and stable error codes are additive optional fields. Storage domain version stays 0; legacy jobs remain readable without fabricated timing. Interrupted accepted jobs become degraded/INTERRUPTED on restart. Diagnostics makes no model calls. Recall events contain IDs actually injected after context packing, never query text, in a global process-local buffer capped at 200 entries; owner filtering returns the last 20. Durable job views return the last 30 while counts cover all records/jobs in the selected owner scope. Whole-document scans and atomic replacement remain architectural limits.
