...
每次新增先提交可召回的 L1 原始记录及 `accepted` 作业。直接模式随后创建一条 L2 事实或 L4 身份信息。抽取模式把不可信对话 JSON 发送给配置模型，只接受完全符合 schema 的严格 JSON 对象，预过滤同一所有者的调和候选，并要求计划恰好覆盖每个抽取来源一次。
...
读取在排名前按所有者、状态、可见性、有效期、请求层与可选 Session 预过滤。画像与普通通道有独立配额。便携提供者组合带版本的 256 维 token/字符哈希向量、BM25 与倒数排名融合；诊断把哈希向量标记为降级语义检索，并报告缺少独立标签通道。
...
包测试使用真实 LLM runtime、storage hub、storage-domain 形式及 JSON 后端，覆盖直接 raw-first 幂等、跨 Session 与仅当前 Session 检索、降级抽取、严格成功抽取及重复证据、证据感知遗忘和四个工具契约。真实 Loader 组合先通过 JSON 后端写入，完整释放后冷启动新组合，并召回上一 Session 的记录。
...
**要求具体嵌入服务。** Harness 当前没有嵌入能力，把本包绑定到 LLM 厂商会破坏提供者接缝。便携哈希空间随显式诊断与导入兼容检查一起交付。
...
参考存储优先保证正确性与便携性，而非规模：所有者状态是一整条 JSON 记录，语义向量只是近似，并发序列化仅限进程内。分页事务提供者、训练型嵌入、原生结构化输出、认证主体映射、保留策略与 L5-L7 语义仍属于未来独立决策。
